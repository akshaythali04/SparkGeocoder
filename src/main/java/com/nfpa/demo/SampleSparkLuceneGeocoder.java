 ...
 */
public class SampleSparkLuceneGeocoder implements Serializable {
    //    private static final String SPARK_MASTER = "local[1]";     // "spark://ClairvoyantAdmin:7077";
    private static final String APP_NAME = "SampleSparkLuceneGeocoder";
    ...
    static Logger logger = Logger.getLogger(SampleSparkLuceneGeocoder.class);
    ...
    public void csvLuceneIndexer(String HDFS_OUTPUT_DIR, String LUCENE_INDEXES_STORE_DIR,
                                 String IP_BUSINESS_ADDRESS, int TOP_RECORD_LIMIT, int NO_OF_PARTITIONS, int RECORD_LIMIT) throws Exception {
        ...
        // JavaRDD<Row> addressRDD = nfpaAddrDf.select("streetname", "state", "city", "zip5", "street_pre", "num_mile", "streettype").toJavaRDD();
        JavaRDD<Row> addressRDD = nfpaAddrDf.select("streetname", "state", "city", "zip5", "street_pre", "num_mile", "streettype").repartition(NO_OF_PARTITIONS).limit(RECORD_LIMIT).toJavaRDD();
...
//test 1       // resultRDD.saveAsTextFile("/Users/Project_Work/NFPA/SparkGeocodeDemo/result_output_test");
    ...
    public static void main(String[] params) throws ParseException, Exception {
        ...
        int RECORD_LIMIT = Integer.parseInt(params[5]);
        long lStartTime = System.currentTimeMillis(); // start time
        logger.info("------------Started---------------" + new Date().toString());
        SampleSparkLuceneGeocoder loadTable = new SampleSparkLuceneGeocoder();
        loadTable.disableLogging();
        loadTable.doInit();
        loadTable.csvLuceneIndexer(HDFS_OUTPUT_DIR, LUCENE_INDEXES_STORE_DIR, IP_BUSINESS_ADDRESS, TOP_RECORD_LIMIT, NO_OF_PARTITIONS, RECORD_LIMIT);
